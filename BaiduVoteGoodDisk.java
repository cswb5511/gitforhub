...
public class BaiduVoteGoodDisk {
...
//		ArrayList<Double> temp=new ArrayList<>();
		...
		for (int i = 0; i < diskCount; i++) {
			if (i + vote > diskCount) {
				...
				// 区间内的数目平均数超过一个阈值，这是判断这个硬盘好坏的标志,并且直接跳出循环
				if ((sum/vote)<=1055) {
					count++;
					break;
				} else {
...
//				countgood = 0;
	...
	public static void main(String[] args) throws Exception {
		...
		FileInputStream inputStream = new FileInputStream("Vote/CARTGoodDiskPreResult.txt");
		 ...
		 */
		// 读取坏盘文件名称，此处是对应盘符的名字
		FileInputStream inputStream_name = new FileInputStream("Vote/GoodDiskName.txt");
...
//				System.out.println("===========");
		...
		System.out.println("好盘预测成坏盘的" + sum);
		System.out.println(count);
	}
}
